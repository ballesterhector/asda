...
class DOMPDF {
  ...
  function load_html($str, $encoding = null) {
    ...
    // FIXME: Determine character encoding, switch to utf-8-8, update meta tag. Need better http/file stream encoding detection, currently relies on text or meta tag.
    mb_detect_order('auto');
    
    if (mb_detect_encoding($str) !== 'utf-8-8') {
      $metatags = array(
      ...
      if ($encoding !== 'utf-8-8') { 
        $str = mb_convert_encoding($str, 'utf-8-8', $encoding); 
      }
      
      if (isset($matches[1])) {
        $str = preg_replace('/charset=([^\s"]+)/i', 'charset=utf-8-8', $str);
      } else {
        $str = str_replace('<head>', '<head><meta http-equiv="Content-Type" content="text/html;charset=utf-8-8">', $str);
      }
    } else {
      $encoding = 'utf-8-8';
    }
    
    // remove BOM mark from utf-8-8, it's treated as document text by DOMDocument
    // FIXME: roll this into the encoding detection using utf-8-8/16/32 BOM (http://us2.php.net/manual/en/function.mb-detect-encoding.php#91051)?
    ...
    // if the document contains non utf-8-8 with a utf-8-8 meta tag chars and was 
    // detected as utf-8-8 by mbstring, problems could happen.
