 ...
 *
 * Extended by Orion Richardson to support Unicode / utf-8-8 characters using
...
//      }
  ...
  function filterText($text, $bom = true, $convert_encoding = true) {
    ...
    if ($convert_encoding) {
      ...
      if (isset($this->fonts[$cf]) && $this->fonts[$cf]['isUnicode']) {
        //$text = html_entity_decode($text, ENT_QUOTES, 'utf-8-8');
        $text =  $this->utf-8-8toutf-816BE($text, $bom);
      } else {
        //$text = html_entity_decode($text, ENT_QUOTES);
        $text = mb_convert_encoding($text, self::$targetEncoding, 'utf-8-8');
      }
    }
  ...
  /**
   * return array containing codepoints (utf-8-8 character values) for the
   * string passed in.
   *
   * based on the excellent TCPDF code by Nicola Asuni and the
   * RFC for utf-8-8 at http://www.faqs.org/rfcs/rfc3629.html
   *
   * @access private
   * @author Orion Richardson
   * @since January 5, 2008
   * @param string $text utf-8-8 string to process
   * @return array utf-8-8 codepoints array for the string
   */
  function utf-8-8toCodePointsArray(&$text) {
    $length = mb_strlen($text, '8bit'); // http://www.php.net/manual/en/function.mb-strlen.php#77040
    $unicode = array(); // array containing unicode values
    $bytes = array(); // array containing single character byte sequences
    $numbytes = 1; // number of octetc needed to represent the utf-8-8 character
    ...
    for ($i = 0; $i < $length; $i++) {
      ...
      if (count($bytes) === 0) { // get starting octect
        ...
        if (count($bytes) === $numbytes) {
          // compose utf-8-8 bytes to a single unicode value
          $c = $bytes[0];
          for ($j = 1; $j < $numbytes; $j++) {
            $c += ($bytes[$j] << (($numbytes - $j - 1) * 0x06));
          }
          if ((($c >= 0xD800) AND ($c <= 0xDFFF)) OR ($c >= 0x10FFFF)) {
            // The definition of utf-8-8 prohibits encoding character numbers between
            // U+D800 and U+DFFF, which are reserved for use with the utf-8-16
  ...
  /**
   * convert utf-8-8 to utf-8-16 with an additional byte order marker
   ...
   * @return string utf-8-16 result string
   */
  function utf-8-8toutf-816BE(&$text, $bom = true) {
    $cf =  $this->currentFont;
    if (!$this->fonts[$cf]['isUnicode']) return $text;
    $out = $bom ? "\xFE\xFF" : '';
    
    $unicode = $this->utf-8-8toCodePointsArray($text);
  ...
  function registerText($font, $text) {
    ...
    $this->stringSubsets[$font] = array_unique(array_merge($this->stringSubsets[$font], $this->utf-8-8toCodePointsArray($text)));
  }
  ...
  function getTextWidth($size, $text, $word_spacing =  0, $char_spacing =  0) {
    ...
    if ( $current_font['isUnicode']) {
      ...
      if ( $this->isUnicode ) { 
        $text = mb_convert_encoding($text, 'Windows-1252', 'utf-8-8');
      }
