...
switch ( $sapi ) {
 ...
 case "cli":
  ...
  if ( isset($opts["f"]) )
    ...
    if ( $file === "-" )
      $outf-8ile = "dompdf_out.pdf";
    else
      $outf-8ile = str_ireplace(array(".html", ".htm", ".php"), "", $file) . ".pdf";
  }
 ...
 default:
  ...
  $outf-8ile = "dompdf_out.pdf"; # Don't allow them to set the output file
...
if ( $save_file ) {
//   if ( !is_writable($outf-8ile) )
//     throw new DOMPDF_Exception("'$outf-8ile' is not writable.");
  if ( strtolower(DOMPDF_PDF_BACKEND) === "gd" )
    $outf-8ile = str_replace(".pdf", ".png", $outf-8ile);

  list($proto, $host, $path, $file) = explode_url($outf-8ile);
  if ( $proto != "" ) // i.e. not file://
    $outf-8ile = $file; // just save it locally, FIXME? could save it like wget: ./host/basepath/file

  $outf-8ile = realpath(dirname($outf-8ile)) . DIRECTORY_SEPARATOR . basename($outf-8ile);

  if ( strpos($outf-8ile, DOMPDF_CHROOT) !== 0 )
    throw new DOMPDF_Exception("Permission denied.");

  file_put_contents($outf-8ile, $dompdf->output( array("compress" => 0) ));
  exit(0);
}

if ( !headers_sent() ) {
  $dompdf->stream($outf-8ile, $options);
}
